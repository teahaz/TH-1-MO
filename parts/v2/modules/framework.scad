...
module rail(make_printable) {
    ...
    mirror([0, 1, 0]) {
        ...
        difference() {
            union() {
                translate([rail_depth, 0, -rail_flat_h]) rotate([0, 180+bottom_angle, 0]) cube([2, fw_exp[1]-side_wall, 2]);
                translate([-2+rail_depth, 0, -rail_flat_h]) cube([2, fw_exp[1]-side_wall, rail_flat_h]);
            }
            translate([-5+rail_depth, 0, 0]) cube([5, fw_exp[1]-side_wall, 5]);
        }
...
module expansion_card_lid() {
    gap = 0.25;
    union() {
        translate([side_wall+gap, side_wall+gap, fw_exp[2]-side_wall]) cube([fw_exp[0]-side_wall*2-gap*2, fw_exp[1]-side_wall*2-gap*2, side_wall]);
        difference() {
            translate([fw_exp[0]/2-usb_c_w/2+gap, fw_exp[1]-side_wall-gap, usb_c_r+usb_c_h]) cube([usb_c_w-gap*2, side_wall+gap, fw_exp[2]-(usb_c_r+usb_c_h)]);
            translate([fw_exp[0]/2, fw_exp[1], usb_c_r+usb_c_h]) usb_c_cutout(false);
        }
    }
}
...
module expansion_card_fw_exp(open_end, make_printable, pcb_mount="boss") {
    // Hollowing of the inside
    extra = 0.1;
    inner = [fw_exp[0]-side_wall*2, fw_exp[1]-side_wall*2, fw_exp[2]-side_wall+extra];
    boss_radius = 2.3;
    ...
    difference() {
        ...
        edge_r = 0.8;
        fillet(edge_r, fw_exp[0]);
        
        // The USB-C plug cutout
        translate([fw_exp[0]/2, fw_exp[1], usb_c_r+usb_c_h]) usb_c_cutout(!open_end);
        
        // The sliding rails
        translate([0, fw_exp[1], rail_h]) rail(make_printable);
        translate([fw_exp[0], fw_exp[1], rail_h]) mirror([1, 0, 0]) rail(make_printable);
        ...
        translate([0, fw_exp[1]-ledge_cut_d, 0]) cube([fw_exp[0], ledge_cut_d, ledge_cut]);
        // The fillet on that cover
        translate([fw_exp[0], fw_exp[1]-ledge_cut_d, 0]) rotate([0, 0, 180]) fillet(ledge_cut/2, fw_exp[0]);
    }
    ...
    if (pcb_mount == "boss" || pcb_mount == "boss_insert") {        
        ...
        translate([fw_exp[0]-boss_x, boss_y, 0]) boss(boss_radius, pcb_mount == "boss_insert", make_printable);
        ...
        translate([fw_exp[0]-boss_x-boss_radius, boss_y-boss_radius, 0]) {
            cube([boss_radius*2, boss_radius*2, pcb_h]);
            if (make_printable)
                translate([boss_radius, boss_radius*2, 0]) rib(boss_radius*2, pcb_h);
...
module FrameworkExpansion() {
    rotate([90, 0, 0])
        translate([0, 0, 0])
        expansion_card_fw_exp(pcb_mount="");
}

module FrameworkVolume() {
